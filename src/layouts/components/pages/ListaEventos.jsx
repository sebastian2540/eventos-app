...
const ListaEventos = () => {
  ...
  return (
    <div className="container">
      <h2>Eventos Registrados <b>|</b> <strong>CESDE</strong></h2>
      {eventos.map((item) => (
        <div key={item.id} className="evento">
          <h3>{item.getNombre}</h3>
          <p><b>Fecha del Evento:</b> {item.getFecha}</p>
          <p><b>Ubicacion del Evento:</b> {item.getUbicacion}</p>
          <p><b>Descripcion del Evento</b> <br />{item.getDescripcion}</p>
        </div>
      ))}
      ;
      <button
